...
# المتغيرات العامة للتحكم في الهجوم اللانهائي
spam_active = False
spam_thread = None
spam_results = None
spam_target = None
...
def infinite_spam(target_uid, regions_to_try, delay_between_requests=0.5):
    global spam_active, spam_results
    
    accounts = load_accounts()
    if not accounts:
        spam_results = {"error": "No accounts found in accounts.txt"}
        spam_active = False
        return
    ...
    while spam_active:
        ...
        # تجهيز التوكين
        token, err = get_jwt_token(acc['uid'], acc['password'])
        if not token:
            results['failed'] += 1
            err_key = f"Token error: {err}" if err else "Token error: Unknown"
            results['response_counts'][err_key] = results['response_counts'].get(err_key, 0) + 1
            account_index += 1
            time.sleep(1)
            continue
        
        # إرسال الطلب على السيرفرات
        success = False
        final_response = None
        ...
        results['total_requests_sent'] = request_number
        account_index += 1
...
def rizer():
    global spam_active, spam_thread, spam_results, spam_target
    ...
    if spam_active:
        return jsonify({
            "error": "هجوم نشط بالفعل!",
            "current_target": spam_target,
            "use /stop لإيقافه"
        }), 409
    ...
    # تحديد المنطقة
    if region_param:
        region_lower = region_param.lower()
        if region_lower in REGION_MAP:
            regions_to_try = [(region_lower, REGION_MAP[region_lower])]
        else:
            return jsonify({"error": f"Invalid region: {region_param}"}), 400
    else:
        regions_to_try = ALL_REGIONS
    
    # تحديد التأخير (بالثواني)
    delay = 0.5
    if delay_param:
        try:
            delay = float(delay_param)
            if delay < 0.1:
                delay = 0.1
        except ValueError:
            pass
    ...
    spam_active = True
    spam_target = target_uid
    spam_results = None
    
    spam_thread = threading.Thread(target=infinite_spam, args=(target_uid, regions_to_try, delay))
    spam_thread.daemon = True
    spam_thread.start()
    
    return jsonify({
        "status": "started",
        "message": f"بدأ الهجوم اللانهائي على الـ UID: {target_uid}",
        "target": target_uid,
        "delay": delay,
        "region": region_param if region_param else "all",
        "use /stop للإيقاف",
        "use /status لمعرفة الإحصائيات"
    })

@app.route('/stop', methods=['GET'])
def stop_spam():
    global spam_active, spam_results, spam_target
    
    if not spam_active:
        return jsonify({"error": "لا يوجد هجوم نشط"}), 400
    
    spam_active = False
    
    # انتظار قليل عشان الخيط يخلص
    time.sleep(1)
    
    return jsonify({
        "status": "stopped",
        "message": "تم إيقاف الهجوم",
        "final_results": spam_results
    })
...
def spam_status():
    ...
    if not spam_active and not spam_results:
        return jsonify({"active": False, "message": "لا يوجد هجوم نشط أو سابق"})
    
    if spam_active and spam_results is None:
        return jsonify({
            "active": True,
            "target": spam_target,
            "success": 0,
            "failed": 0,
            "total_sent": 0,
            "message": "الهجوم بدأ للتو، انتظر قليلاً..."
        })
    
    if spam_active and spam_results:
        return jsonify({
            "active": True,
            "target": spam_results.get("target_uid"),
            "success": spam_results.get("success", 0),
            "failed": spam_results.get("failed", 0),
            "total_sent": spam_results.get("total_requests_sent", 0),
            "response_counts": spam_results.get("response_counts", {}),
            "status": spam_results.get("status", "running")
        })
    
    # الهجوم متوقف
    return jsonify({
        "active": False,
        "last_results": spam_results,
        "message": "الهجوم متوقف. use /rizer لبدء هجوم جديد"
    })
...
def index():
    return jsonify({
        "tool": "FreeFire Friend Request Spammer",
        "endpoints": {
            "/rizer?uid=ID&region=optional&delay=optional": "بدء هجوم لا نهائي",
            "/stop": "إيقاف الهجوم",
            "/status": "معرفة حالة الهجوم الحالي",
            "/health": "فحص صحة السيرفر"
        },
        "note": "ضع الحسابات في ملف accounts.txt بصيغة uid:password"
    })

if __name__ == "__main__":
    print("""
    ╔════════════════════════════════════════╗
    ║   FreeFire Friend Request Spammer      ║
    ║   هجوم لا نهائي - يشتغل حتى توقفه      ║
    ╚════════════════════════════════════════╝
    """)
    print("[+] Server running on http://0.0.0.0:5000")
    print("[+] ضع الحسابات في ملف accounts.txt")
    print("[+] استخدم /rizer?uid=الهدف لبدء الهجوم")
    print("[+] استخدم /stop لإيقاف الهجوم")
    print("[+] استخدم /status لمعرفة الإحصائيات")
    print("")
